...
def carr_madan_integrand_factor_space(
    ...
    """
    Carr-Madan integrand for IQFT-based option pricing.
    
    CORRECTED FORMULA:
    ψ(u) = exp(-α·k) · exp(-rT) · φ(u - i(α+1)) / (α² + α - u² + iu(2α+1))
    
    where k = log(K/S₀) is log-strike.
    
    This is the modified characteristic function that, after IQFT,
    gives the option price distribution.
    
    Parameters
    ----------
    u : np.ndarray
        Frequency grid (real values)
    char_func_values : np.ndarray (complex)
        Characteristic function φ(u - i(α+1)) already evaluated
        at damped frequencies
    damping_alpha : float
        Damping parameter (typically 1.5 for call options)
    risk_free_rate : float
        Risk-free rate
    maturity : float
        Time to maturity
    log_strike : float
        Log-strike k = log(K/S₀)
        
    Returns
    -------
    np.ndarray (complex)
        Carr-Madan integrand values ψ(u)
        
    Notes
    -----
    The integrand includes:
    1. Damping factor exp(-αk) to ensure integrability
    2. Discount factor exp(-rT)
    3. Characteristic function at damped frequency
    4. Carr-Madan denominator for call option
    ...
    # Full integrand with all terms
    integrand = (
        np.exp(-alpha * log_strike) *
        np.exp(-risk_free_rate * maturity) *
        char_func_values / denom
    )
